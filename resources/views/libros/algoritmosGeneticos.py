...
def SeleccionRangos(poblacion,fitness,cant):
    nuevaPob=[]
    #ordenar a la poblacion (ascendente-maximizar descendente-minimizar)
    #pasadas
    for i in range(len(fitness)-1):
        #comparaciones
        j=0
        while(j<((len(poblacion)-1)-i)):
            if(fitness[j]>fitness[j]):
                aux=fitness[j]
                ...
                poblacion[j+1]=aux
            j+=1
    #Obtener la suma total de los rankings
    sumTotalRanking=0
    for i in range(1,len(poblacion)+1):
        sumTotalRanking+=1
    #obtener la prob de selección de cada individuo
    probSeleccion=[]
    for fit in fitness:
        probSeleccion.append(i/sumTotalRanking)
    #generamos los rangos de seleccion para cada individuo    
    rangoSeleccion=[0]
    for valor in range(10):
        #Establecer los rangos de selección
        if valor==0:
            rangoSeleccion.append(probSeleccion[valor])
        else:
            rangoSeleccion.append(probSeleccion[valor]+rangoSeleccion[valor-1]) #El valor -1 nos ayuda a regresar al nuemro anterior para ver si hay un valor y sumarlo
    while(len(nuevaPob)<cant):
        #Simulamos la seleccion de un individuo
        aleatorio=random.random()
        for i in range(10):
            if aleatorio<=rangoSeleccion[i]:
                nuevaPob.append(poblacion[i])
                break
    return nuevaPob
...
while(generacion<4):
    ...
    poblacion = SeleccionRangos(poblacion,fitness,10)
